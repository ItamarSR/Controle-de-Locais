<?php
// Página de impressão em lote (sem layout).
...
<body>
  <?php foreach (($locals ?? []) as $local): ?>
    <div class="page">
      <div class="etq">
        <div class="nome"><?= htmlspecialchars((string)($local['nome_local'] ?? '')) ?></div>
        <div class="mp">
          MP: <?= htmlspecialchars((string)($local['codigo_mp'] ?? '')) ?>
          <small><?= htmlspecialchars((string)($local['nome_mp'] ?? '')) ?></small>
        </div>
      </div>
    </div>
  <?php endforeach; ?>
